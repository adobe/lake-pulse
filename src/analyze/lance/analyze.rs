...
mod tests {
    ...
    mod integration {
        ...
        async fn test_schema_evolution_real_dataset() {
            ...
            // Get the table URI
            let table_uri = storage.uri_from_path("");

            // Analyze schema evolution
            let result = analyzer.analyze_schema_evolution(&table_uri).await;
        ...
        async fn test_deletion_metrics_real_dataset() {
            ...
            // Analyze deletion metrics
            let result = analyzer.analyze_deletion_metrics(&table_uri).await;
            ...
            assert!(
                impact_score >= 0.0 && impact_score <= 1.0,
                "Impact score should be between 0 and 1"
            );
        ...
        async fn test_table_analyzer_trait_implementation() {
            ...
            let referenced = <LanceAnalyzer as TableAnalyzer>::find_referenced_files(
                &analyzer,
                &metadata_files,
            )
            .await;
